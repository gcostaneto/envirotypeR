                        ...
                        sleep = 60)
{
 ...
 #   message("NASA POWER: Done!")
  ...
  if(isTRUE("CDD10" %in% variables.names) & isTRUE('n' %in% variables.names))
  {
    # THI2 = AT + (0.36 × DP) + 41.2.
    .output_weather$PTU_CDD <- .output_weather$CDD10*.output_weather$n

    variables.names <- c(variables.names,'PTU_CDD')
    comp1 <-'x'
  }

  message(paste0('Computing PTU_CDD...................[ ',comp1,' ]'))
  comp1 <-''
